...
class Miner extends Child
{
     ...
     * @param $pgsql_pool
     */
    public function __construct($pgsql_pool = null)
    {
        parent::__construct();
        $this->option = array();
        $this->select = array();
     ...
     * @return $this
     */
    public function setPool(PgsqlAsynPool $pgsql_pool)
    {
        $this->core_name = $pgsql_pool->getAsynName();
        $this->pgsql_pool = $pgsql_pool;
        $this->clear();
        return $this;
    }
    ...
    {
        //Pgsql协程处理，不能传入链接，暂时不能事务。
        $result = null;
        $this->pgsql_pool->getSync()->pdoBeginTrans();
        try {
            $result = $fuc(null);
            $this->pgsql_pool->getSync()->pdoCommitTrans();
        } catch (\Throwable $e) {
            $this->pgsql_pool->getSync()->pdoRollBackTrans();
            if ($errorFuc != null) {
    ...
    {
        $pgsqlCoroutine = Pool::getInstance()->get(PgsqlCoroutine::class);
        if (get_instance()->isTaskWorker()) {//如果是task进程自动转换为同步模式
            $this->mergeInto($this->pgsql_pool->getSync());
            $this->clear();
            $data = $this->pgsql_pool->getSync()->pdoQuery();
            return new PgsqlSyncHelp(null, $data);
        } else {
            $statement = $this->getStatement();
            $holder = $this->getPlaceholderValues();
            $sql = $this->getStatement(false);
            $this->clear();
            $pgsqlCoroutine->setRequest($sql);
            if ($set) {
                $set($pgsqlCoroutine);
            }
            $result = $this->pgsql_pool->prepare($sql, $statement, $holder, $this->client, $pgsqlCoroutine);
            return $result;
        }
    }
    ...
    public function _query($sql = null, callable $set = null)
    {
        ...
        if (get_instance()->isTaskWorker()) {//如果是task进程自动转换为同步模式
            $this->mergeInto($this->pgsql_pool->getSync());
            $this->clear();
            $data = $this->pgsql_pool->getSync()->pdoQuery($sql);
            ...
            if ($sql != null) {
                ...
                $result = $this->pgsql_pool->query($sql, $this->client, $pgsqlCoroutine);
                return $result;
            } else {
                $sql = $this->getStatement(false);
                $this->clear();
    ...
    public function pdoConnect($activeConfig)
    {
        ...
        $dsn = sprintf("pgsql:host=%s port=%s dbname=%s user=%s password=%s", $activeConfig['host'], $activeConfig['port'], $activeConfig['database'], $activeConfig['user'], $activeConfig['password']);
        $pdo = new \PDO($dsn);
